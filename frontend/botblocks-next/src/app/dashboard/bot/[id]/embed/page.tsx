...
></iframe>`}
                    ...
                    <button
                        onClick={() => copyToClipboard(`<iframe src="http://localhost:3000/widget/${botId}" style="position: fixed; bottom: 20px; right: 20px; width: 400px; height: 600px; border: none; z-index: 9999;" title="Chat Widget"></iframe>`)}
    ...
    window.addEventListener('message', function(e) {
      if (e.data && e.data.type === 'BOTBLOCKS_RESIZE') {
        ...
        iframe.style.pointerEvents = 'auto'; // Simple approach, or manage carefully
                    ...
                    <button
                        onClick={() => {
                            const code = `<script>(function(){const iframe=document.createElement('iframe');const botId='${botId}';iframe.src='http://localhost:3000/widget/'+botId;iframe.style.cssText='position:fixed;bottom:0;right:0;width:150px;height:160px;border:none;z-index:9999;transition:all 0.3s ease;color-scheme:none;background:transparent;';iframe.title='Chat Widget';document.body.appendChild(iframe);window.addEventListener('message',function(e){if(e.data&&e.data.type==='BOTBLOCKS_RESIZE'){iframe.style.width=e.data.width;iframe.style.height=e.data.height;}});})();</script>`;
